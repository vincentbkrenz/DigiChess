...
void printMoveAndBoard() {
  Serial.print(mn);
  if (k == 0x08) Serial.print(". ");  // White just moved
  else             Serial.print("... "); // Black just moved
  Serial.println(c);
  serialBoard();
}
...
void waitForEnter() {
  ...
  // clear out any leftover input
  while (Serial.available()) {
    Serial.read();
  }
  // now block until we see a newline
  while (true) {
    if (Serial.available()) {
      if (Serial.read() == '\n') {
        break;
...
void bkp() {
  for (int i = 0; i < 16*8+1; i++) bk[i] = b[i];
}
...
// Returns the last move as a String
String getLastMove() {
  return String(c);
}
